...
#         qs = db.session.query(FolioScheme.scheme.distinct().label('scheme'),FolioScheme.folio).filter(FolioScheme.id.in_(scheme_ids)).order_by(FolioScheme.scheme).all()
...
#                     continue
#                 try :
#                     nav_history = NAVHistory(scheme=sid.folio, date=date, nav= item["nav"])
